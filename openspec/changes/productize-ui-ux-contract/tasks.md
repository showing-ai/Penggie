...
- [ ] 5.5 Validate that terminal footers, model/status rows, active input rows, slash/resume/approval surfaces, and transient startup/status UI do not become sealed assistant transcript content.
- [ ] 5.6 Validate fallback behavior: low-confidence rendering preserves visible terminal text, exposes traceability/fallback reason where practical, and keeps Raw Terminal audit available.
...
- [ ] 8.1 Review `PenggieTheme.swift`, `PenggieThemeController.swift`, `PenggieRootView.swift`, and `PenggieGhosttySession.swift` against the token layer, scene token, component token, and renderer palette contract.
...
- [ ] 8.6 Run or add static checks that prevent new scattered `windowBackgroundColor`, `textBackgroundColor`, hardcoded terminal background hex, or feature-view raw color use unless explicitly allowed.
...
- [ ] 9.3 Add fixture README entries describing source terminal facts, expected Display AST, expected Reading rendering, expected selected-row evidence, and expected low-confidence behavior.
