...
- Evidence bundle `logs/running-build-identity.txt` result generated by
  `scripts/qa/check-product-ui-ux-live-qa-status.sh --strict --evidence-dir <bundle>`,
  proving the live process loaded the bundle's `Penggie.debug.dylib` path and
  inode.
...
- Persisted `logs/running-build-identity.txt` evidence from the strict live QA
  status command, not only terminal output copied into review notes.
