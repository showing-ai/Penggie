...
struct ShowCLITerminalScreenSnapshot: Codable, Equatable {
...
struct ShowCLINativeInteractionLine: Identifiable, Equatable {
    ...
    func withSelection(_ selected: Bool) -> Self {
        .init(screenLineIndex: screenLineIndex, text: text, isSelected: selected)
    }
}

enum ShowCLINativeInteractionScope {
    static func hasActiveContinuation(
        in snapshot: ShowCLITerminalScreenSnapshot?,
        minimumScreenLineIndex: Int? = nil,
        excludingRowsUnchangedFrom previousSnapshot: ShowCLITerminalScreenSnapshot? = nil
    ) -> Bool {
        !rows(
            from: snapshot,
            currentInput: "",
            minimumScreenLineIndex: minimumScreenLineIndex,
            excludingRowsUnchangedFrom: previousSnapshot
        ).isEmpty
    }

    static func inputAnchorScreenLineIndex(
        in snapshot: ShowCLITerminalScreenSnapshot?,
        currentInput: String
    ) -> Int? {
        guard let snapshot, !currentInput.isEmpty else { return nil }

        let lines = snapshot.lines.sorted { $0.index < $1.index }
        ...
        return lines[anchorIndex].index
    }

    static func rows(
        from snapshot: ShowCLITerminalScreenSnapshot?,
        currentInput: String,
        minimumScreenLineIndex: Int? = nil,
        excludingRowsUnchangedFrom previousSnapshot: ShowCLITerminalScreenSnapshot? = nil
    ) -> [ShowCLINativeInteractionLine] {
    ...
    private static func filteredLines(
        _ lines: [ShowCLITerminalScreenSnapshot.Line],
        minimumScreenLineIndex: Int?,
        excludingRowsUnchangedFrom previousSnapshot: ShowCLITerminalScreenSnapshot?
    ) -> [ShowCLITerminalScreenSnapshot.Line] {
    ...
    private static func selectedLineIndex(
        in lines: [ShowCLITerminalScreenSnapshot.Line],
        ...
        if let undimmedIndex = undimmedMenuSelectionLineIndex(
            in: lines,
            startingAt: startIndex,
            requiresContinuationMenuStructure: requiresContinuationMenuStructure
        ) {
            return undimmedIndex
        }
    ...
    private static func inputAnchorIndex(
        ...
        if let cursorY {
            ...
            .first(where: { isInputAnchor(lines[$0].text, currentInput: currentInput) })
        }
    ...
    private static func rowsAfterInputAnchor(
        ...
        var rows: [ShowCLINativeInteractionLine] = []
    ...
    private static func currentInputSuggestionRows(
        _ rows: [ShowCLINativeInteractionLine],
        currentInput: String
    ) -> [ShowCLINativeInteractionLine] {
        ...
        var result: [ShowCLINativeInteractionLine] = []
    ...
    private static func screenLine(
        _ line: ShowCLITerminalScreenSnapshot.Line,
...
enum ShowCLINativeInteractionPresentation {
    static func rows(
        from snapshot: ShowCLITerminalScreenSnapshot?,
        phase: ShowCLINativeInteractionPhase,
        currentInput: String,
        minimumScreenLineIndex: Int? = nil,
        excludingRowsUnchangedFrom previousSnapshot: ShowCLITerminalScreenSnapshot? = nil
    ) -> [ShowCLINativeInteractionLine] {
        ...
        case .editing:
            ...
            return ShowCLINativeInteractionScope.rows(
                from: snapshot,
                currentInput: currentInput,
                excludingRowsUnchangedFrom: previousSnapshot
            )
    ...
    static func usesNativeComposerInput(phase: ShowCLINativeInteractionPhase) -> Bool {
        phase.ownsComposerInput
    }

    static func standardComposerIsEnabled(phase: ShowCLINativeInteractionPhase) -> Bool {
        !phase.isActive
    }
}
#endif
